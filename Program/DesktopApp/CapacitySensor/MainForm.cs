...
namespace CapacitySensor
{
    public partial class MainForm : Form
    {
        ...
        private void LoadFormTheme()
        {
            switch (TCTRL.SelectedTab.Text)
            {
                case "Generals":
                    ...
                    BTN_ExportTXT.BackColor = Theme.PrimaryColor;
        ...
        private void UpdateCapacityChart()
        {
            ...
            ChartCapacity.ChartAreas[0].AxisY.Maximum = Math.Floor(cap_max) + corr;
            ChartCapacity.ChartAreas[0].AxisY.Minimum = Math.Ceiling(cap_min) - corr;
        ...
        private void BTN_ExportTXT_Click(object sender, EventArgs e)
        {
            ...
            string FileName = $"Logs-{now.Hour}{now.Minute}{now.Second}.txt";
            string Path = PathDesktop + "\\" + FileName;
            Logs.Create(Logs.From.User, Logs.Type.Info, 
                $"Log TXT Document Generated - Desktop\\{FileName}");
            string Text = " From    Type     Date                          Message\n" +
                TBOX_FullLogs.Text;
        ...
        private void BTN_MatlabLast_Click(object sender, EventArgs e)
        {
            Matlab.Generate();
        }
